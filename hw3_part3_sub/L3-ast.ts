...
;; <define> ::= ( define <var> <cexp> )     / DefExp(var:NewVarDec, val:CExp)
;; <var> ::= <identifier>                   / VarRef(var:string)
;; <cexp> ::= <number>                      / NumExp(val:number)
;;         |  <boolean>                     / BoolExp(val:boolean)
;;         |  <string>                      / StrExp(val:string)
;;         |  ( lambda ( <var>* ) <cexp>+ ) / ProcExp(params:NewVarDec[], body:CExp[]))
...
export interface DefineExp {tag: "DefineExp"; var: NewVarDec; val: CExp; };
...
export interface ProcExp {tag: "ProcExp"; args: NewVarDec[], body: CExp[]; };
export interface Binding {tag: "Binding"; var: NewVarDec; val: CExp; };
...
export const makeDefineExp = (v: NewVarDec, val: CExp): DefineExp =>
...
export const makeProcExp = (args: NewVarDec[], body: CExp[]): ProcExp =>
    ({tag: "ProcExp", args: args, body: body});
export const makeBinding = (v: NewVarDec, val: CExp): Binding =>
...
const parseProcExp = (sexps: any[]): ProcExp | Error =>
    safeFL((body: CExp[]) => makeProcExp( map((sexp) => sexp instanceof Array && second(sexp) === "lazy" ? makeLazyVarDec(sexp[0]) : makeVarDecl(sexp), sexps[1]), body))
...
export const parseLazyDecls = (sexps: any[]): LazyVarDec[] | Error =>
allT(isString, sexps) ? map(makeLazyVarDec, sexps) :
Error(`LazyVarDecl must be a string - ${sexps}`);
...
const safeMakeBindings = (decls: NewVarDec[] | Error, vals: Array<CExp | Error>): Binding[] | Error =>
    isError(decls) ? decls :
